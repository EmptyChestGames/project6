...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        if self.launchMarker.split then
            local a, b = self.launchMarker.alien2.body:getLinearVelocity()
            local c, d = self.launchMarker.alien3.body:getLinearVelocity()
            local aPos, bPos = self.launchMarker.alien3.body:getPosition()
            local cPos, dPos = self.launchMarker.alien3.body:getPosition()

            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                if aPos < 0 or (math.abs(a) + math.abs(b) < 1.5) then
                    if cPos < 0 or (math.abs(c) + math.abs(d) < 1.5) then 
