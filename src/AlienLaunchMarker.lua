...
function AlienLaunchMarker:init(world)
    ...
    -- check to see if the player has split the aliens
    self.split = false

    -- flag for checking if the inital alien has collided with anything
    self.collided = false
...
function AlienLaunchMarker:update(dt)
    ...
    elseif self.launched == true and self.split == false and love.keyboard.isDown('space') and self.collided == false then
        local x, y = self.alien.body:getPosition(self.alien)
        self.alien2 = Alien(self.world, 'round', x, y - 30, 'Player')
        self.alien2.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 12)
        self.alien3 = Alien(self.world, 'round', x, y + 30, 'Player')
        self.alien3.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 12)
